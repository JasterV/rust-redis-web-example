use crate::models::{R2D2Error::*, Result};
use r2d2_redis::{
...
pub fn connect(url: &'static str) -> Result<r2d2::Pool<RedisConnectionManager>> {
    let manager = RedisConnectionManager::new(url).map_err(RedisClientError)?;
    r2d2::Pool::builder()
